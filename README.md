...
10. **2026-09-25 — Where do sessions live, and how can two Tasks share context?** AX has no Session resource: a coordinator or agent program owns the conversation and explicitly passes versioned artifacts and summaries between isolated Tasks. [Sessions live above Tasks](sessions-live-above-tasks.md).
11. **2026-09-25 — What is the coordinator in practice, and does `ax apply` just launch subagents?** The coordinator is your own trusted process and session store; AX gives it sandbox creation and lifecycle, not decomposition or result synthesis. [Physical coordinator and AX interface](brain-to-ax-interface.md).
12. **2026-09-25 — Is AX basically a simplified Kubernetes?** Only its declarative CLI is similar: AX is an additional agent-sandbox control layer that depends on Kubernetes and Agent Substrate, not a replacement. [AX is not Kubernetes-lite](ax-is-not-kubernetes-lite.md).
13. **2026-09-25 — What concrete workload benefits from AX?** A large fleet of mostly idle, isolated PR agents can keep per-Task workspaces while Substrate multiplexes active actors onto warm workers; the coordinator still owns sessions and idle policy. [Mostly idle PR agents](mostly-idle-pr-agents.md).
